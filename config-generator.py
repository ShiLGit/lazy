...
def prompt_until_valid(prompt_text, all_options):
    keep_prompting = True 
    while keep_prompting:
        inpt = input(prompt_text)
        inpt = inpt.strip()
        if inpt in all_options:
            return inpt 
        else: 
...
def generate_versionmap(rows):
    ...
    for currentv in cvers: 
        ...
        if not fv: 
            ...
            fv = prompt_until_valid(f"Choose a fix version for from above for {currentv} or ENTER to apply no changes: ", all_fv.union(set([''])))
            fv = fv.strip()
        if len(fv)> 0:
